...
extern int manual_flag;                    //Denote when the manual mode is enabled, is reset when any other mode is on
extern int auto_pause_flag;                //To pause the execution of the auto mode
extern int auto_stop_flag;                 //To stop the execution of the auto mode
...
extern float state_value;                //Used for storing the time duration between 2 commands while controlling the bot in manual mode
...
/* Funtion that forms the HTML page */
char* default_page();
char* choose_page();
char* get_sta();
...
/* Funtions used when the ESP boots or when file data is to update variables from file */
esp_err_t update_number(int n);
esp_err_t update_wifi();
esp_err_t update_paths();
esp_err_t update_pathname();

/* Funtions used to update the file when deletion or replacement is performed*/
